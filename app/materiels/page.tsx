...
import { useData, Materiel } from "../context/DataContext";
...
const Materiels: React.FC = () => {
  ...
  const stats = {
    ...
    disponible: materiels.filter(
      (m) => m.statut?.toLowerCase() === "disponible"
    ).length,
    enUtilisation: materiels.filter(
      (m) => m.statut?.toLowerCase() === "en utilisation"
    ).length,
    maintenance: materiels.filter(
      (m) => m.statut?.toLowerCase() === "maintenance"
    ).length,
  };
  return (
    <Box
