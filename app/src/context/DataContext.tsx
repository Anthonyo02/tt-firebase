...
// ✅ Import Firebase
import { db } from "../firebase"; // Vérifie le chemin !
import {
  collection,
  getDocs,
  addDoc,
...
export const DataProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // 🔥 garder seulement les non-admin (comme ton code précédent)
      const employerUsers = formattedUsers.filter((u) => u.role !== "admin");
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        formattedMateriels.push({
          ...
          imageUrl: m.imageUrl || "", // Peut être une URL ou du Base64 stocké
        });
      });
  ...
  const addMateriel = async (materiel: Omit<Materiel, "id">) => {
    const newMaterielData = {
      ...
      imageUrl: materiel.imageUrl || "", // ✅ URL CLOUDINARY
    };
  ...
  const deleteMateriel = async (id: string) => {
    ...
    try {
      ...
      addToOfflineQueue({ id, type: "materiel", action: "delete", data: null });
    }
  };
