...
const UsersListModal: React.FC<UsersListModalProps> = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "users", deleteUser.id)); // Supprime le doc dans Firestore
      setDeleteUser(null);
      // Pas besoin de mettre à jour localUsers → Firestore snapshot le fera automatiquement
    } catch (err) {
      console.error(err);
