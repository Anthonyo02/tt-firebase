// components/realisation/helpers.ts

import { CLIENT_OPTIONS, TECHNOLOGY_OPTIONS, THEME } from "./constants";
...
export const getClientLabel = (clientValue: string): string => {
  const found = CLIENT_OPTIONS.find((c) => c.value === clientValue);
  return found?.label || clientValue;
};

export const getTechLabel = (techValue: string): string => {
  const found = TECHNOLOGY_OPTIONS.find((t) => t.value === techValue);
  return found?.label || techValue;
};